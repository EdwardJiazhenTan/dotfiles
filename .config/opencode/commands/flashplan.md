---
description: Plan a Linear ticket (or PRD) into a Haiku-executable spec
---
...
- A Linear ticket ID like `DEV-4478` → fetch with the Linear MCP `get_issue` tool.
- Free-form PRD text → use as-is.
- Empty → ask the user to provide a ticket ID or paste a PRD. Do not proceed without input.

## Step 2 — Ask clarifying questions liberally
...
- Existing patterns in the affected package (especially in `galatea` — see Step 4).

## Step 4 — Convention vs. best-practice flagging
...
Present the plan in the conversation in this exact structure:
...
- **Never modify files.** Present the plan in the conversation only.
